      ...
      if(err) return console.log("Error viewing Resources");
        ...
        for(i=  0; i < items.length; i++) {
            ...
            page += "<div>"
               ...
               + "<span class='remove'><a href='#' class='delR' rel='" + items[i].url + "'>Delete</a></span>";
...
router.post('/uploadResource', function(req, res, next) {
  ...
  if(done == true)
  {
    ...
    resources.uploadResource(req.files.resource, req.body.description); 
...
router.post('/removeType', function(req, res, next) {
    ...
    resources.removeResourceType(req.body.Type);
    res.location("./");
    res.redirect("./"); 
});
